...
- **Replaced the default Flutter launch placeholder** (Flutter warned about it on every
  ...
  `#0A0A0F`**, matching the app icon.
- **Logo source:** `assets/splash/splash_logo.png`, generated by **`tool/gen_splash.py`**
  (teal "FC" over the purple→teal "150" with a soft glow, on a transparent canvas — the
  sibling of `tool/gen_icon.py`).
- **Wiring:** `flutter_native_splash` (dev dep) with config in `pubspec.yaml`
  (`color: "#0A0A0F"`, `image: assets/splash/splash_logo.png`, `android_12` block). Applied
  with **`dart run flutter_native_splash:create`**, which regenerates the iOS
  `LaunchScreen.storyboard` + `LaunchImage`/`LaunchBackground` asset sets and the Android
  `launch_background.xml` / `values-v31` (Android 12) drawables. Re-run that command after
  regenerating the PNG. iOS-only release for now, but Android assets are generated too.
